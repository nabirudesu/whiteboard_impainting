...
sys.path.append(str(Path(__file__).resolve().parents[1] / 'python/openvino/model_zoo'))
...
class ImreadWrapper(ImagesCapture):
    ...
    def __init__(self, input, loop):
        ...
        print('this performance debugging for imread')
        ...
        if self.image is None:
            raise OpenError("Can't open the fucking image from {}".format(input))
...
class DirReader(ImagesCapture):
    ...
    def __init__(self, input, loop):
        ...
        self.loop = loop
        self.reader_metrics = PerformanceMetrics()
        print('this performance debugging for imdir')
        self.dir = input
